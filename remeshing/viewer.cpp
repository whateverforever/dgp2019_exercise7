...
Viewer::Viewer() : nanogui::Screen(Eigen::Vector2i(1024, 768), "DGP Viewer") {
    ...
    b->setCallback([this]() {
        ...
        mesh_->set_target_length(0.002f);
        if(textBox_targetlength != NULL)
            textBox_targetlength->setValue("0.002");
        this->refresh_mesh();
    ...
    b->setCallback([this]() {
        ...
        mesh_->set_target_length(2.0f);
        if(textBox_targetlength != NULL) textBox_targetlength->setValue("2");
        this->refresh_mesh();
        this->refresh_trackball_center();
    });
    ...
    {
        new Label(window_, "Target Length :", "sans-bold");
        textBox_targetlength =  new TextBox(window_);
        textBox_targetlength->setEditable(true);
        textBox_targetlength->setFixedSize(Vector2i(100, 20));
        textBox_targetlength->setValue("2");
        textBox_targetlength->setDefaultValue("0.0");
        textBox_targetlength->setFontSize(16);
        textBox_targetlength->setFormat("[-]?[0-9]*\\.?[0-9]+");
        textBox_targetlength->setCallback([&](const std::string& str)->bool{
            mesh_->set_target_length(std::stof(str));
            return true;
        });
    }
    ...
    vector<string> remeshing_type = {"Uniform", "Adaptive"};
	...
	b = new Button(window_, "Tangential Relaxation");
	b->setCallback([this]() {
    ...
    mesh_->set_target_length(2.0f); // add this line
